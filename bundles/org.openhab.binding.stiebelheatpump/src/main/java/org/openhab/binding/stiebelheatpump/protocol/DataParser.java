    ...
    public Map<String, String> parseRecords(final byte[] response, Request request) throws StiebelHeatPumpException {
        ...
        for (RecordDefinition recordDefinition : request.getRecordDefinitions()) {
            try {
                ...
                logger.debug("Parsed value {} ->\t{}\t\t\t with pos: {}\tlen: {}\tbitPos: {}\tscale: {}", channel,
                        value, recordDefinition.getPosition(), recordDefinition.getLength(),
                        recordDefinition.getBitPosition(), recordDefinition.getScale());
